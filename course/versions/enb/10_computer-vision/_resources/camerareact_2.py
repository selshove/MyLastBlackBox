...
def capture_loop():
    ...
    face_timeout = 1.0  # seconds before sending stop if no face
    last_sent_command = None  # To prevent repeat commands
        ...
        if len(rects) > 0:
            ...
            for (x, y, w, h) in rects:
                ...
                for (x, y, w, h) in rects:
                    ...
                    face_distance_threshold = 50  # ← tune this value (e.g., 80–150)
                    
                    current_time = time.time()
                    
                    if current_time - last_command_time > command_interval:
                        if h < face_distance_threshold:  # face is small → far
                            if rx > frame_w // 2 + 50:
                                ...
                                if last_sent_command != 'f':
                                    ser.write(b'f')  # face is centered → move forward
                                    last_sent_command = 'f'
                        else:
                            if last_sent_command != 'x':
                                ser.write(b'x')  # face is close enough → stop
                                last_sent_command = 'x'
